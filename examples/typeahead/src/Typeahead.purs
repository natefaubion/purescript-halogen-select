...
type TEffects e = (now :: NOW | Effects e)

component :: ∀ m e
  . MonadAff (TEffects e) m
...
component =
  ...
  where
    ...
    render :: State -> H.ParentHTML Query (ChildQuery (TEffects e)) ChildSlot m
    render st =
      HH.div
    ...
    eval :: Query ~> H.ParentDSL State Query (ChildQuery (TEffects e)) ChildSlot Void m
    eval = case _ of
      Log str a -> a <$ do
        H.liftAff $ log str
